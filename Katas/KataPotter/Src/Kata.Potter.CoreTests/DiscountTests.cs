  ...
  public class DiscountTests
  {
    ...
    public void Two_Different_Books_Should_Satisfy_Specification()
    {
      IList<Book> books = Builder<Book>.CreateListOfSize(2)
        .WhereTheFirst(1).IsConstructedWith("First Title", 10M).Have(x => x.IsDiscounted = false)
        .WhereTheLast(1).IsConstructedWith("Second Title", 10M).Have(x => x.IsDiscounted = false)
        .Build();
    ...
    {
      IList<Book> books = Builder<Book>.CreateListOfSize(2)
        .WhereAll().AreConstructedWith("Title", 10M)
        .Build();
    ...
    public void Three_Different_Books_Should_Satisfy_Specification()
    {
      IList<Book> books = Builder<Book>.CreateListOfSize(3)
        .WhereTheFirst(1).IsConstructedWith("First Title", 10M)
        .WhereAll().AreConstructedWith("Second Title", 10M).Have(x => x.IsDiscounted = false)
        .WhereTheLast(1).IsConstructedWith("Third Title", 10M)
        .Build();
      new Discount(2, 0).IsSatisfiedBy(books).ShouldBeTrue();
    }
    ...
    public void Three_Books_Where_Two_Are_The_Same_Book_Should_Satisfy_Specification()
    {
      IList<Book> books = Builder<Book>.CreateListOfSize(3)
        .WhereTheFirst(1).IsConstructedWith("First Title", 10M).Have(x => x.IsDiscounted = false)
        .WhereTheLast(2).IsConstructedWith("Second Title", 10M).Have(x => x.IsDiscounted = false)
        .Build();
      new Discount(2, 0).IsSatisfiedBy(books).ShouldBeTrue();
    }
    ...
    {
      IList<Book> books = Builder<Book>.CreateListOfSize(3)
        .WhereAll().AreConstructedWith("Title", 10M).Have(x => x.IsDiscounted = false)
        .Build();
      new Discount(2, 0).IsSatisfiedBy(books).ShouldBeFalse();
    }
  }
